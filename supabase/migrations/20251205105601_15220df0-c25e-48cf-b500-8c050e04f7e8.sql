-- Fix the stop_all_user_trades function to not reference non-existent stopped_at column
...
BEGIN
  ...
  UPDATE profiles 
  SET 
    base_balance = COALESCE(base_balance, 0) + v_total_profit + v_total_initial_amount,
    net_balance = COALESCE(net_balance, 0) + v_total_profit + v_total_initial_amount,
    interest_earned = COALESCE(interest_earned, 0) + v_total_profit,
    total_invested = GREATEST(0, COALESCE(total_invested, 0) - v_total_initial_amount)
  WHERE id = p_user_id;
